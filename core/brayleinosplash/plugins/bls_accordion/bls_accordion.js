    ...
    if (btn.classList.contains('active')) {
      ...
      if (curr[0]) {
        ...
        instance.elements.panel.map((x) => {
          ...
          x.style.height = `${x.querySelector('.bls-panel__body').offsetHeight}px`;
          return x;
        });
      }
    } else {
...
function _bindMethodClickOpen(instance) {
  if (window.blsAccordion !== undefined) {
    window.blsAccordion.map((x) => {
  ...
  */
  constructor(setting) {
    const defaultSetting = {
      selector: 'button[data-bls-accordion]',
      events: {
        initialized() {},
        initializedAll() {},
        beforeOpen() {},
  ...
  init(setting) {
    ...
    if (window.blsAccordion === undefined) window.blsAccordion = [];

    els.map((x) => {
      const _f = window.blsAccordion.filter(y => y.element === x);
      if (_f.length === 0) {
        const obj = {};
        const s = Object.assign({}, $this.setting, x.dataset || {});
        obj.setting = s;

        const panel = convertNodeListToArray(document.querySelectorAll(`.bls-panel${x.dataset.target}`));
        const siblings = convertNodeListToArray(document.querySelectorAll(`.bls-accordion__heading[data-group="${x.dataset.group}"]`));
