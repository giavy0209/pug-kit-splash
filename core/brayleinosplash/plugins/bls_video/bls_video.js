import './bls_video.scss';
import '../../polyfill';
import { convertNodeListToArray, wrap, getUrlQueryString, getSelectors } from '../../utils';
import BlsProgress from '../bls_progress/bls_progress';

/* Private Method */
const _eventReady = new CustomEvent('blsVideo.Ready', { bubbles: false, cancelable: false });
const _eventLoad = new CustomEvent('blsVideo.Load', { bubbles: false, cancelable: false });
...
function _buildUI(instance, index) {
  ...
  // building class
  _instance.elements.player.classList.add('bls-video__player');

  if (_instance.setting.type === 'youtube') {
    const iframe = document.createElement('div');
    iframe.id = `bls-video__player${index}`;
  ...
  if (_instance.setting.controls) {
    ...
    _btnPlay.classList.add('bls-video__play');

    _btnVolume.type = 'button';
    _btnVolume.classList.add('bls-video__mute');

    _btnFullScreen.type = 'button';
    _btnFullScreen.classList.add('bls-video__fullscreen');

    _elProgress.value = 0;
    if (_instance.elements.player.duration) _elProgress.max = _instance.elements.player.duration;
    _elProgress.classList.add('bls-video__progress');

    _elVolume.value = 0.5;
    _elVolume.max = 1.0;
    _elVolume.classList.add('bls-video__volume');
    ...
    _controls.classList.add('bls-video__controls');
    _controls.appendChild(_fragControls);

    _instance.elements.wrapper.appendChild(_controls);
    const _progress = new BlsProgress({
      selector: 'progress.bls-video__progress',
    })[0];

    const _volume = new BlsProgress({
      selector: 'progress.bls-video__volume',
      events: {
...
function _bindEvents(instance) {
  ...
  instance.elements.wrapper.addEventListener('blsVideo.Ready', () => {
    if (typeof instance.setting.events.ready === 'function') instance.setting.events.ready(instance);
  }, false);
  instance.elements.wrapper.addEventListener('blsVideo.Load', () => {
...
class BlsVideo {
  ...
  init(setting) {
    ...
    els.map((x, index) => {
      ...
      wrapper.classList.add('bls-video');
      wrap(wrapper, x);
      obj.elements = {
        wrapper,
        player: x,
