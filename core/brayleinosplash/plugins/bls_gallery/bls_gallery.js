import './bls_gallery.scss';
import BlsModal from '../bls_modal/bls_modal';
...
function _showContent(instance, img) {
  if (instance.setting.type === 'image') {
    if (instance.elements.contentMainTarget.children.length === 0) {
      instance.elements.contentMainTarget.innerHTML = `<img data-bls-lazy src="${instance.setting.loading}" alt="${img}" data-source="${img}" />`;
    } else {
      instance.elements.contentMainTarget.children[0].src = img;
    }
    if (typeof instance.setting.events.afterSlide === 'function') instance.setting.events.afterSlide(instance);
    const lazies = new BlsLazyElement({
      selector: `.${instance.elements.modal.className.split(' ').join('.')} [data-bls-lazy]:not(.loaded)`,
    });
    return lazies;
  }
...
function _contentGoPrev(instance) {
  ...
  if (typeof instance.setting.events.beforeSlide === 'function') instance.setting.events.beforeSlide(instance);
...
function _contentGoNext(instance) {
  ...
  if (typeof instance.setting.events.beforeSlide === 'function') instance.setting.events.beforeSlide(instance);
  if (instance.setting.thumbnail && instance.data.sources.length > 0) {
  ...
  el.addEventListener('click', (e) => {
    if (e.target.nodeName === 'IMG') {
      ...
      if (instance.setting.type === 'image') {
        ...
        _showContent(instance, instance.data.sources[instance.data.currentIndex].src);
      }
    }
  });
...
function _buildGroupEvent(instance) {
  ...
  instance.elements.btn.addEventListener('click', (e) => {
    const matches = e.target.matches ? e.target.matches(instance.setting.children) : e.target.msMatchesSelector(instance.setting.children);
    if (!matches) return;
...
function _buildPhotoList(instance) {
  if (!instance.elements.photoList) {
    ...
    div.innerHTML = html;
    div.className = 'bls-thumbnail__wrapper';
    instance.elements.modalContent.querySelector('.bls-modal__body').appendChild(div);
...
function _buildMainContentUI(instance) {
  if (!instance.elements.contentMain) {
    const div = document.createElement('div');
    // div.id = 'blsMainPhoto';
    div.classList.add('bls-content__main');
    div.innerHTML = '<div class="bls-content__target"></div><div class="bls-content__nav"><button type="button" title="Previous" class="bls-content__button bls-content__button--prev"></button><button type="button" title="Next" class="bls-content__button bls-content__button--next"></button></div>';
    instance.elements.modalContent.querySelector('.bls-modal__body').appendChild(div);
    instance.elements.contentMain = div;
    instance.elements.contentMainTarget = div.querySelector('.bls-content__target');
    instance.elements.contentMainNav = div.querySelector('.bls-content__nav');
    instance.elements.contentMainNavPrev = div.querySelector('.bls-content__button--prev');
    instance.elements.contentMainNavNext = div.querySelector('.bls-content__button--next');
...
function _buildGalleryEvent(instance) {
  if (instance.setting.type === 'ajax') {
    const _gallerybeforeOpen = instance.setting.events.beforeOpen;
    instance.setting.events.beforeOpen = function _newGalleryBeforeOpen(_instance) {
      ...
      if (typeof _gallerybeforeOpen === 'function') _gallerybeforeOpen(_instance);
    };
  }

  const _galleryAfterOpen = instance.setting.events.afterOpen;
  instance.setting.events.afterOpen = function _newGalleryAfterOpen(_instance) {
    if (_instance.setting.thumbnail) {
      ...
      if (_instance.data.currentIndex !== undefined && _instance.setting.type === 'image') {
        _div.querySelectorAll('img')[_instance.data.currentIndex].click();
      }
    } else if (_instance.data.currentIndex !== undefined && _instance.setting.type === 'image') {
      _showContent(_instance, _instance.data.sources[_instance.data.currentIndex].src);
    }
    if (typeof _galleryAfterOpen === 'function') _galleryAfterOpen(_instance);
  };
...
class BlsGallery extends BlsModal {
  constructor(setting) {
    const defaultSetting = {
      selector: '[data-bls-gallery]',
      type: 'image',
      display: 'fullscreen',
      scrollOverlay: false,
      thumbnail: true,
    ...
    s.thumbnail = (s.thumbnail === '1' || s.thumbnail === 'true' || s.thumbnail === 1 || s.thumbnail === true);
  ...
  init(setting) {
    ...
    $this.instances.map((x) => {
      ...
      obj.setting.thumbnail = (obj.setting.thumbnail === '1' || obj.setting.thumbnail === 'true' || obj.setting.thumbnail === 1 || obj.setting.thumbnail === true);

      obj.data = obj.data || {};
      obj.data.currentIndex = 0;
      obj.data.sources = [];

      obj.elements.modal.classList.add('bls-modal--gallery');

      if (obj.setting.contentTarget === null || obj.setting.contentTarget === undefined) {
        obj.setting.contentTarget = 'bls-content__target';
      }
