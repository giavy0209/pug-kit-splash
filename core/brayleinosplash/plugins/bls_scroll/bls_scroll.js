...
import './bls_scroll.scss';
import {
  getSelectors, debounce, wrap, unWrap,
} from '../../utils';

const eventReachTop = new CustomEvent('blsScroll.ToTop', { bubbles: false, cancelable: false });
const eventReachBottom = new CustomEvent('blsScroll.ToBottom', { bubbles: false, cancelable: false });
const eventReachLeft = new CustomEvent('blsScroll.ToLeft', { bubbles: false, cancelable: false });
const eventReachRight = new CustomEvent('blsScroll.ToRight', { bubbles: false, cancelable: false });
...
function _buildUIScrollbar(instance) {
  ...
  if (_instance.setting.type === 'vertical' || _instance.setting.type === 'both') {
    ...
    _barV.className = 'bls-scroll__bar bls-scroll__bar--vertical';
    _thumbV.className = 'bls-scroll__thumb bls-scroll__thumb--vertical';
  ...
  if (_instance.setting.type === 'horizontal' || _instance.setting.type === 'both') {
    ...
    _barH.className = 'bls-scroll__bar bls-scroll__bar--horizontal';
    _thumbH.className = 'bls-scroll__thumb bls-scroll__thumb--horizontal';
...
function _buildUI(instance) {
  ...
  wrapper.classList.add('bls-scroll');
  wrapper.setAttribute('tabindex', 0);

  const scroll = document.createElement('div');
  scroll.className = _instance.setting.class || '';
  scroll.classList.add('bls-scroll__container');
  ...
  if (window.app.Device.type === 'mobile') {
    wrapper.classList.add('bls-scroll__mobile');
  }
...
function _bindEvents(instance) {
  ...
  if (window.blsScroll !== undefined) {
    window.blsScroll.map((x) => {
...
class BlsScroll {
  ...
  */
  ...
  init(setting) {
    ...
    if (window.blsScroll === undefined) window.blsScroll = [];
    els.map((x) => {
      const _f = window.blsScroll.filter(y => y.element === x);
      if (_f.length === 0) {
        const obj = {};
