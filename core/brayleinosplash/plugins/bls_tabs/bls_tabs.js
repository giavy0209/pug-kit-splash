...
const _clickEventHandle = (instance, e) => {
  ...
  if (e.target.classList.contains('bls-tab__link')) {
    if (!li.classList.contains('active')) {
      const curr = instance.elements.nav.ul.querySelector('.bls-tab__item.active');
...
class BlsTab {
  ...
  */
  ...
  init(setting) {
    ...
    if (window.blsTab === undefined) window.blsTab = [];
    els.map((x) => {
      const _f = window.blsTab.filter(y => y.element === x);
      if (_f.length === 0) {
        const obj = {};
        ...
        const lis = convertNodeListToArray(x.querySelectorAll('.bls-tab__link'));
        const panels = [];
        lis.map((y) => {
          const p = document.querySelector(`.bls-panel${y.getAttribute('href')}`);
          if (p !== null) {
            panels.push(p);
          }
          return y;
