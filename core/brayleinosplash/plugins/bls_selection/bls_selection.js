...
import '../../polyfill';
import {
  convertNodeListToArray, fireEvent, unWrap, getSelectors,
} from '../../utils';
...
const _changeEventHandle = (instance) => {
  if (instance.elements) {
    if (instance.setting.multiple) {
      ...
      const _selected = instance.elements.options.querySelector('.selected');
      if (_selected !== null) {
        instance.elements.btn.innerText = instance.elements.options.querySelector('.selected').innerText;
      } else {
        instance.elements.btn.innerText = instance.elements.select.children[instance.elements.select.selectedIndex].innerText;
      }
    }
  }
};
...
class BlsSelection {
  ...
  */
  constructor(setting) {
    ...
    s.mobile = (s.mobile === '1' || s.mobile === 'true' || s.mobile === 1 || s.mobile === true);
    this.setting = s;
    this.instances = [];
    this.init(s);
  ...
  init(setting) {
    ...
    els.map((x) => {
      ...
      if (_f.length === 0) {
        ...
        if (!obj.setting.mobile) wrapper.classList.add('bls-mobile--native');
